...
<main>
  ...
  <div class="table-wrap">
    <table>
      ...
      <tbody id="funnel-tbody"><tr><td colspan="10" class="empty">Введите токены и нажмите «Загрузить»</td></tr></tbody>
    </table>
  </div>
...
function showInfo(msg){document.getElementById('info-banner').style.display='block';document.getElementById('info-banner').textContent=msg;document.getElementById('error-banner').style.display='none';}
...
async function loadAll() {
  ...
  try {
    ...
    if (!tokenData.token) throw new Error('Токен не настроен на сервере. Задайте AMO_ACCESS_TOKEN.');
    ...
    const stages={};
    pipelinesRaw.forEach(p=>(p._embedded?.statuses||[]).forEach(s=>stages[s.id]={name:s.name,pipeline:p.name,pipelineId:p.id}));
...
function renderDashboard(leads,newL,tasks,users,stages) {
  ...
  // ID этапов воронки НОВОСТРОЙКИ 2.0 (pipeline_id: 10391694)
  const S = {
    QUAL:    82143450, // 3. Квалифицирован
    MEET1:   82143454, // 4. Встреча назначена
    MEET2:   82143458, // 5. Встреча проведена
    SHOW1:   82143462, // 6. Показ назначен
    SHOW2:   83396946, // 7. Показ проведен
    BOOK:    82143466, // 8. Бронь ОБЪЕКТА
    DDU:     82143470, // 9. ПОДПИСАНИЕ ДДУ
  };
  const ACTIVE_IDS = new Set(Object.values(S));
  const isActive = sid => ACTIVE_IDS.has(sid);
  const isQual   = sid => ACTIVE_IDS.has(sid); // 3+ это и есть activeLeads
  ...
  const activeLeads=leads.filter(l=>isActive(l.status_id));
  ...
  // Воронка — по ID этапов с правильными названиями
  const STAGE_NAMES = {
    82143450: '3. Квалифицирован',
    82143454: '4. Встреча назначена',
    82143458: '5. Встреча проведена',
    82143462: '6. Показ назначен',
    83396946: '7. Показ проведен',
    82143466: '8. Бронь объекта',
    82143470: '9. Подписание ДДУ',
  };
  const stageTotals={};
  ...
  const stageOrder=[82143450,82143454,82143458,82143462,83396946,82143466,82143470];
  const funnelKeys=stageOrder.filter(id=>stageTotals[id]);
  document.getElementById('funnel-wrap').innerHTML=funnelKeys.map(id=>`
...
function renderLeads(leads){
  ...
  tbody.innerHTML=filtered.sort((a,b)=>b.created_at-a.created_at).slice(0,200).map(l=>{
    ...
    const sname=s[l.status_id]?.name||'—';
    const sc=sname.match(/^[3-9]\./)?'badge-ok':sname.includes('Архив')?'badge-danger':'badge-muted';
    return `<tr>
...
function openBroker(uid) {
  ...
  const STAGE_NAMES_DD = {
    ...
    82143466:'8. Бронь объекта', 82143470:'9. Подписание ДДУ', 142:'11. Комиссия получена', 143:'~ Архив'
  };
  const ACTIVE_IDS_DD = new Set([82143450,82143454,82143458,82143462,83396946,82143466,82143470]);

  const bAll    = allLeads.filter(l=>l.responsible_user_id===uid);
  const bActive = bAll.filter(l=>ACTIVE_IDS_DD.has(l.status_id));
  ...
  const stageOrderDD=[82141386,82141390,82141394,82143450,82143454,82143458,82143462,83396946,82143466,82143470,142,143];
  const stageKeys=stageOrderDD.filter(id=>stageCounts[id]);
  const stagesHtml=stageKeys.map(id=>`
    <div class="stage-bar">
      <div class="stage-bar-name">${STAGE_NAMES_DD[id]||stages[id]?.name||`ID:${id}`}</div>
  ...
  const leadsHtml=bNew.sort((a,b)=>b.created_at-a.created_at).slice(0,100).map(l=>{
    ...
    const ds=d.toLocaleDateString('ru-RU',{day:'2-digit',month:'2-digit'})+' '+d.toLocaleTimeString('ru-RU',{hour:'2-digit',minute:'2-digit'});
    const tags=l._embedded?.tags?.map(t=>t.name).join(', ')||'—';
    const sname=STAGE_NAMES_DD[l.status_id]||stages[l.status_id]?.name||'—';
    const sc=ACTIVE_IDS_DD.has(l.status_id)?'badge-ok':l.status_id===143?'badge-danger':'badge-muted';
    return `<tr>
      <td style="font-family:'IBM Plex Mono',monospace;font-size:11px;color:var(--muted2)">${ds}</td>
      <td style="color:var(--muted2);font-size:11px">${tags}</td>
